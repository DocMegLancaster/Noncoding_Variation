# %%
import os
import csv
import time
import traceback
import threading
import multiprocessing
import queue
import random
import requests
import numpy as np
from pathlib import Path
from dotenv import load_dotenv
import sys

load_dotenv(dotenv_path=Path(__file__).parent / ".env")

#Edit:
# run_one_piece function for specific gene

GENE_STRAND = "+"
GENE_NAME   = "DSP"
CHROM       = "chr6"

ISM_START = int(sys.argv[1])
ISM_END = int(sys.argv[2])

CHUNK_BP        = 500   # bp per score_ism_variants() call
SEQ_LENGTH      = 1_048_576 #model input/context window
CALL_TIMEOUT_S  = 1000
MAX_WORKERS     = 16    # parallelism on AG's end. internal score_ism_variants concurrency per call
TISSUE_ONTOLOGY = "UBERON:0002084"  # LV

# Ensembl REST is flaky under load -- read timeouts and transient 500s both show
# up mid-run and used to kill the whole region. Retry those with backoff.
ENSEMBL_TIMEOUT_S  = 120
ENSEMBL_ATTEMPTS   = 8
ENSEMBL_BACKOFF_S  = 5    # base delay; doubles each attempt, capped below
ENSEMBL_MAX_WAIT_S = 120
SCORES_CSV = str(Path(__file__).parent / f"dsp_ism_out/dsp_ism_{ISM_START}_{ISM_END}_{SEQ_LENGTH}.csv")

# Dynamically picks up every non-empty AG_API_KEY_<N> env var present, in
# numeric order -- add or remove keys in .env and the worker count follows,
API_KEYS = [v for _, v in sorted(
    ((k, os.environ[k].strip()) for k in os.environ
     if k.startswith("AG_API_KEY_") and k.removeprefix("AG_API_KEY_").isdigit()),
    key=lambda kv: int(kv[0].removeprefix("AG_API_KEY_")),
) if v]


#Note: To add a scorer/modality, _build_scorers() and extract_score() will need updating

# RECOMMENDED columns are pulled straight from RECOMMENDED_VARIANT_SCORERS;
# everything else is COMPLEMENT, even if it happens to match the recommended one.
# AG client defines MAX_VARIANT_SCORERS_PER_REQUEST = 20
# and recommended scorers = 19

def _build_scorers():
    from alphagenome.models import dna_client, variant_scorers

    complement = {
        # "SPLICE_SITES": variant_scorers.CenterMaskScorer(
        #     requested_output=dna_client.OutputType.SPLICE_SITES, width=501,
        #     aggregation_type=variant_scorers.AggregationType.DIFF_SUM),
        # "SPLICE_SITE_USAGE": variant_scorers.CenterMaskScorer(
        #     requested_output=dna_client.OutputType.SPLICE_SITE_USAGE, width=501,
        #     aggregation_type=variant_scorers.AggregationType.DIFF_LOG2_SUM),
    }

    scorers = []
    for key, scorer in variant_scorers.RECOMMENDED_VARIANT_SCORERS.items():
        scorers.append((f"{key}, RECOMMENDED", key, scorer))
        if key in complement:
            scorers.append((f"{key}, COMPLEMENT", key, complement[key]))
    return scorers


# %%
def fetch_reference_sequence(chrom_bare: str, start_1based: int, end_1based: int) -> str:
    """Fetch from Ensembl REST, retrying transient failures.

    Retries read/connect timeouts, dropped connections, malformed JSON, and the
    retryable HTTP codes (429 rate-limit, 5xx server errors). A 4xx other than
    429 means the request itself is wrong (bad region, bad chrom), so it fails
    immediately rather than burning attempts.
    """
    url = (f"https://rest.ensembl.org/sequence/region/human/"
           f"{chrom_bare}:{start_1based}-{end_1based}")
    retryable_status = {429, 500, 502, 503, 504, 408}

    for attempt in range(1, ENSEMBL_ATTEMPTS + 1):
        try:
            r = requests.get(url, headers={"Content-Type": "application/json"},
                             timeout=ENSEMBL_TIMEOUT_S)
            if r.status_code in retryable_status:
                # Ensembl sends Retry-After on 429; respect it when present.
                retry_after = r.headers.get("Retry-After")
                wait = float(retry_after) if retry_after and retry_after.replace(".", "", 1).isdigit() \
                    else min(ENSEMBL_BACKOFF_S * 2 ** (attempt - 1), ENSEMBL_MAX_WAIT_S)
                reason = f"HTTP {r.status_code}"
            else:
                r.raise_for_status()
                return r.json()["seq"].upper()
        except (requests.exceptions.Timeout,
                requests.exceptions.ConnectionError,
                requests.exceptions.ChunkedEncodingError,
                ValueError, KeyError) as e:
            # ValueError covers a truncated/non-JSON body, KeyError a response
            # without "seq" -- both happen when Ensembl half-answers.
            wait = min(ENSEMBL_BACKOFF_S * 2 ** (attempt - 1), ENSEMBL_MAX_WAIT_S)
            reason = f"{type(e).__name__}: {e}"

        if attempt == ENSEMBL_ATTEMPTS:
            raise RuntimeError(
                f"Ensembl fetch failed after {ENSEMBL_ATTEMPTS} attempts for "
                f"{chrom_bare}:{start_1based}-{end_1based} -- last failure: {reason}"
            )
        wait += random.uniform(0, 0.25 * wait)   # jitter, so parallel jobs desync
        print(f"  [ensembl] attempt {attempt}/{ENSEMBL_ATTEMPTS} failed "
              f"({reason}); retrying in {wait:.1f}s", flush=True)
        time.sleep(wait)

def build_all_variant_rows(ref_seq: str, start_1based: int, score_cols: list) -> list:
    blank_scores = {c: "" for c in score_cols}
    return [
        {"position": start_1based + i, "ref": ref, "alt": alt, **blank_scores}
        for i, ref in enumerate(ref_seq)
        for alt in ([b for b in "ACGT" if b != ref] if ref in "ACGT" else list("ACGT"))
    ]

# extract_score's shape guards double as emptiness guards: an empty adata.X
# yields an all-False col_mask/row_mask below, which the .any() checks catch.
def extract_score(adata, output_type_name: str) -> float:
    X, var = adata.X, adata.var
    if output_type_name == "SPLICE_SITES":
        col_mask = ((var["name"] == "donor") | (var["name"] == "acceptor")) & \
                   (var["strand"] == GENE_STRAND)
    elif output_type_name == "SPLICE_JUNCTIONS":
        col_mask = (var["ontology_curie"] == TISSUE_ONTOLOGY) & \
                   (var["Assay title"] == "total RNA-seq")
    elif output_type_name == "SPLICE_SITE_USAGE":
        col_mask = (var["ontology_curie"] == TISSUE_ONTOLOGY) & \
                   (var["Assay title"] == "total RNA-seq") & \
                   (var["strand"] == GENE_STRAND)
    elif output_type_name == "CONTACT_MAPS":
        col_mask = np.ones(X.shape[1], dtype=bool)
    elif "ontology_curie" in var.columns:
        col_mask = (var["ontology_curie"] == TISSUE_ONTOLOGY)
    else:
        col_mask = np.ones(X.shape[1], dtype=bool)
    col_mask = col_mask.values if hasattr(col_mask, "values") else col_mask
    if not col_mask.any():
        return float("nan")

    obs = adata.obs
    if output_type_name == "CONTACT_MAPS":
        row_mask = np.ones(X.shape[0], dtype=bool)
    else:
        row_mask = (obs["gene_name"] == GENE_NAME).values if "gene_name" in obs.columns \
            else np.ones(X.shape[0], dtype=bool)
    if not row_mask.any():
        return float("nan")

    sub = np.asarray(X)[np.ix_(row_mask, col_mask)].flatten().astype(float)
    finite = sub[np.isfinite(sub)]
    if finite.size == 0:
        return float("nan")
    return float(finite[np.argmax(np.abs(finite))])

def run_one_piece(api_key, chunk_start, chunk_end, out_queue):
    from alphagenome.data import genome
    from alphagenome.models import dna_client

    scorers = _build_scorers()
    model = dna_client.create(api_key)
    ism_interval = genome.Interval(CHROM, chunk_start, chunk_end)
    DSP_INTERVAL = genome.Interval('chr6', 7_541_325, 7_586_717)
    seq_interval = DSP_INTERVAL.resize(SEQ_LENGTH)

    print(f"  [worker] start {CHROM}:{chunk_start}-{chunk_end}  "
          f"ism={ism_interval}  seq={seq_interval}  "
          f"n_scorers={len(scorers)}", flush=True)

    t0 = time.time()
    try:
        result = model.score_ism_variants(
            interval=seq_interval,
            ism_interval=ism_interval,
            variant_scorers=[s for _, _, s in scorers],
            organism=dna_client.Organism.HOMO_SAPIENS,
            progress_bar=False,
            max_workers=MAX_WORKERS,
            merge_stranded_gene_tracks=False,
        )
        partial = {}
        for variant_scores in result:
            v = variant_scores[0].uns["variant"]
            key = (v.position, v.reference_bases, v.alternate_bases)
            row = {}
            for (col_name, ot_name, _scorer), adata in zip(scorers, variant_scores):
                score = extract_score(adata, ot_name)
                row[col_name] = "NaN" if np.isnan(score) else score
            partial[key] = row
        elapsed = time.time() - t0
        print(f"  [worker] ok {CHROM}:{chunk_start}-{chunk_end}  "
              f"{len(partial)} variants  {elapsed:.1f}s", flush=True)
        out_queue.put({"status": "ok", "elapsed": elapsed, "partial": partial})
    except Exception as e:
        tb = traceback.format_exc()
        print(f"  [worker] ERROR {CHROM}:{chunk_start}-{chunk_end}:\n{tb}", flush=True)
        out_queue.put({
            "status": "error",
            "elapsed": time.time() - t0,
            "partial": {},
            "error": f"{type(e).__name__}: {e}",
            "traceback": tb,
        })

def key_dispatcher(worker_id, api_key, my_chunks, rows_by_position,
                    lock, write_csv_fn, counters):
    ctx = multiprocessing.get_context("spawn")

    for chunk_start, chunk_end in my_chunks:
        print(f"  [key {worker_id}] starting {CHROM}:{chunk_start}-{chunk_end} "
              f"(waiting up to {CALL_TIMEOUT_S}s)...", flush=True)
        out_queue = ctx.Queue()
        p = ctx.Process(target=run_one_piece,
                         args=(api_key, chunk_start, chunk_end, out_queue),
                         daemon=True)
        p.start()
        try:
            result = out_queue.get(timeout=CALL_TIMEOUT_S)
        except queue.Empty:
            result = {"status": "timeout", "elapsed": CALL_TIMEOUT_S, "partial": {}}
        p.join(timeout=5)
        if p.is_alive():
            p.terminate()

        ok = result["status"] == "ok"
        with lock:
            if ok:
                for (pos, ref, alt), row in result["partial"].items():
                    for r in rows_by_position.get(pos, []):
                        if r["ref"] == ref and r["alt"] == alt:
                            r.update(row)
                write_csv_fn()
            counters["ok" if ok else "failed"] += 1
            done = counters["ok"] + counters["failed"]

        status = result["status"]
        if status == "ok":
            status_msg = f"ok  {result['elapsed']:.1f}s"
        elif status == "timeout":
            status_msg = f"TIMEOUT after {CALL_TIMEOUT_S}s"
        else:
            err = result.get("error", "unknown error")
            status_msg = f"ERROR -- {err}"
            tb = result.get("traceback")
            if tb:
                print(f"  [key {worker_id}] traceback for "
                      f"{CHROM}:{chunk_start}-{chunk_end}:\n{tb}", flush=True)
        print(f"  [key {worker_id}] {CHROM}:{chunk_start}-{chunk_end}  "
              f"{status_msg}  ({done}/{counters['total']})", flush=True)

def _write_csv(rows_list, score_cols):
    tmp = SCORES_CSV + ".tmp"
    with open(tmp, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["position", "ref", "alt"] + score_cols)
        w.writeheader()
        w.writerows(rows_list)
    os.replace(tmp, SCORES_CSV)

def main() -> None:
    scorers = _build_scorers()
    score_cols = [c for c, _, _ in scorers]

    print("=" * 60, flush=True)
    print("ISM debug config:", flush=True)
    print(f"  API_KEYS         = {len(API_KEYS)}", flush=True)
    print(f"  CHROM / ISM      = {CHROM}:{ISM_START}-{ISM_END}", flush=True)
    print(f"  CHUNK_BP         = {CHUNK_BP}", flush=True)
    print(f"  SEQ_LENGTH       = {SEQ_LENGTH}", flush=True)
    print(f"  CALL_TIMEOUT_S   = {CALL_TIMEOUT_S}", flush=True)
    print(f"  MAX_WORKERS      = {MAX_WORKERS}", flush=True)
    print(f"  TISSUE_ONTOLOGY  = {TISSUE_ONTOLOGY}", flush=True)
    print(f"  SCORES_CSV       = {SCORES_CSV}", flush=True)
    print(f"  scorers ({len(score_cols)}): {score_cols}", flush=True)
    print("=" * 60, flush=True)

    if not Path(SCORES_CSV).exists():
        print(f"Fetching reference sequence ({CHROM}:{ISM_START+1}-{ISM_END})...",
              flush=True)
        ref_seq = fetch_reference_sequence(CHROM.replace("chr", ""), ISM_START + 1, ISM_END)
        rows_list = build_all_variant_rows(ref_seq, ISM_START + 1, score_cols)
        _write_csv(rows_list, score_cols)
        print(f"Pre-initialised {SCORES_CSV} with {len(rows_list):,} rows "
              f"({len(ref_seq):,} positions x 3 alts).", flush=True)
    else:
        with open(SCORES_CSV, newline="") as f:
            reader = csv.DictReader(f)
            missing_cols = [c for c in score_cols if c not in reader.fieldnames]
            if missing_cols:
                raise SystemExit(
                    f"{SCORES_CSV} doesn't have column(s) {missing_cols} -- its "
                    f"scorer config doesn't match the current _build_scorers(). "
                    f"Delete the file or change the region to start fresh."
                )
            rows_list = [{**r, "position": int(r["position"])} for r in reader]
        print(f"Loaded existing {SCORES_CSV} ({len(rows_list):,} rows).", flush=True)

    rows_by_position: dict = {}
    for r in rows_list:
        rows_by_position.setdefault(r["position"], []).append(r)

    def chunk_is_done(cs, ce):
        return not any(r[c] == ""
                        for pos in range(cs + 1, ce + 1)
                        for r in rows_by_position.get(pos, [])
                        for c in score_cols)

    chunks = [(p, min(p + CHUNK_BP, ISM_END)) for p in range(ISM_START, ISM_END, CHUNK_BP)]
    todo = [c for c in chunks if not chunk_is_done(*c)]
    print(f"{len(chunks)} chunks total | {len(chunks) - len(todo)} already done | "
          f"{len(todo)} to run\n", flush=True)
    if not todo:
        print("All chunks complete.", flush=True)
        return

    lock = threading.Lock()
    write_csv_fn = lambda: _write_csv(rows_list, score_cols)
    key_chunks = {i: todo[i::len(API_KEYS)] for i in range(len(API_KEYS))}
    counters = {"ok": 0, "failed": 0, "total": len(todo)}

    print(f"Launching {len(API_KEYS)} key-dispatchers over {len(todo)} chunks "
          f"({CHUNK_BP} bp each)...", flush=True)
    for i in range(len(API_KEYS)):
        print(f"  key {i + 1}: {len(key_chunks[i])} chunks", flush=True)
    print(flush=True)
    run_start = time.time()

    threads = [
        threading.Thread(
            target=key_dispatcher,
            args=(i + 1, key, key_chunks[i], rows_by_position, lock, write_csv_fn, counters),
            daemon=True,
        )
        for i, key in enumerate(API_KEYS)
    ]
    for t in threads:
        t.start()
    for t in threads:
        t.join()

    elapsed_min = (time.time() - run_start) / 60
    print(flush=True)
    print("=" * 60, flush=True)
    print(f"Finished in {elapsed_min:.1f} min | {counters['ok']} chunks ok | "
          f"{counters['failed']} chunks left blank", flush=True)
    print("Re-run the script to retry the blank cells." if counters["failed"]
          else "All chunks complete.", flush=True)

if __name__ == "__main__":
    main()


